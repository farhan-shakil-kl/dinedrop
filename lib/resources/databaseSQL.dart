  ...
  Future<bool> insertData(Food food) async {
    await database.transaction((txn) async {
      int id1 = await txn.rawInsert(
          'INSERT INTO cartTable(keys, name, price,menuId,image,discount,description) VALUES("${food.id}","${food.name}","${food.price}","${food.quantity}","${food.image}","${food.rating}","${food.description}")');
      print('inserted1: $id1');
    });
    return true;
  }
